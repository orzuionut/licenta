...
module.exports = function (io) 
{
    io.on('connection', function (socket) 
    {
        ...
        /////////////////////////
        socket.on('store file chunk', storeFileChunk);
    });

};

function storeFileChunk(data)
{
    if(typeof arrayBufferChunks[data.hash] == 'undefined')
    {
        arrayBufferChunks[data.hash] = [];
    }

    arrayBufferChunks[data.hash].push(data.data);
}
...
function storeFile(data)
{
    ...
    let file = getFileFromArrayBuffer(data.hash, data.fileName);
    
    Event.fire('file.upload', {file: file, path: path});
    Event.fire('file.uploaded', info);
}

function getFileFromArrayBuffer(hash, fileName)
{
    let blob = Buffer.concat(arrayBufferChunks[hash]);
...
function sendBlobToClient(blob, socket, offset)
{
    ...
    if (offset < totalFileSize)
    {
        ...
        setTimeout(sendBlobToClient, 0, blob, socket, offset + chunkSize);
    }
    else
    {
